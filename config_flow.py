...
def get_schema(defaults: dict | None = None) -> vol.Schema:
    """Centralized schema with your original descriptions via strings.json."""
    if defaults is None: defaults = {}
    return vol.Schema({
        vol.Required("grid_sensor", default=defaults.get("grid_sensor")): selector.EntitySelector(
            selector.EntitySelectorConfig(domain="sensor")
        ),
        vol.Required("soc_sensor", default=defaults.get("soc_sensor")): selector.EntitySelector(
            selector.EntitySelectorConfig(domain="sensor")
        ),
        vol.Required("solar_sensor", default=defaults.get("solar_sensor")): selector.EntitySelector(
            selector.EntitySelectorConfig(domain="sensor")
        ),
        vol.Required("inverter_limit_entity", default=defaults.get("inverter_limit_entity")): selector.EntitySelector(
...
class InverterControllerConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def async_get_options_flow(config_entry: config_entries.ConfigEntry) -> InverterControllerOptionsFlowHandler:
        """Fixed: Do not pass arguments here."""
